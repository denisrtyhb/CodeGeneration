...
def make_collate_fn(tokenizer):
    def collate_fn(batch):
        ...
        # try:
        tokenized_3 = tokenizer(list(strings3), padding=True, truncation=True, max_length=256, return_tensors="pt")
        
        tokenized_4 = tokenizer(list(strings4), padding=True, truncation=True, max_length=256, return_tensors="pt")
        # except:
        #     while True:
        #         exec(input(">>> "))
        # Print max sequence lengths for monitoring
        # max_tokens_1 = tokenized_1['input_ids'].size(1)
        # max_tokens_2 = tokenized_2['input_ids'].size(1)
        max_tokens_3 = tokenized_3['input_ids'].size(1)
        max_tokens_4 = tokenized_4['input_ids'].size(1)
